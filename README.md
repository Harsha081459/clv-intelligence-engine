...
A complete Linux CI run on the real workbook is archived in [results/validation_summary.json](results/validation_summary.json), with code revision, package versions and the raw-workbook SHA-256. [Source CI run](https://github.com/Harsha081459/clv-intelligence-engine/actions/runs/35317238950).

The run loaded **1,067,371 rows**, retained **776,624 cleaned rows**, and evaluated **638 test customers**, separate from **2,974 training** and **637 calibration** customers (seed 42, five Optuna trials).

| Model | Test MAE (INR) | Test RMSE (INR) | Pearson r |
|---|---:|---:|---:|
| BG/NBD + Gamma-Gamma | 102,621 | 188,260 | 0.9533 |
| LightGBM | 104,081 | 363,000 | 0.7945 |
| Ridge stacker | 89,887 | 183,877 | 0.9530 |

The stacker reduced test MAE by **12.4%** versus the probabilistic baseline in this run. Split-conformal intervals achieved **91.5% empirical test coverage** at a 90% target, with mean width **INR 358,979**. Wide intervals and high percentage errors remain limitations; these results do not establish general superiority or future-period performance.
